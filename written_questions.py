...
def get_new_elements():
    r = requests.get(base_url + "/ar/%D8%A7%D9%84%D8%A3%D8%B3%D9%80%D8%A6%D9%84%D8%A9-%D8%A7%D9%84%D9%83%D8%AA%D8%A7%D8%A8%D9%8A%D8%A9")
...
def main(init = False):
    ...
    if init:
        new_pd.to_csv(config.written_questions_csv)
        return

    prev_pd = pd.read_csv(config.written_questions_csv)
    diff = get_diff(prev_pd, new_pd)

    if not diff.empty:
        for index, row in diff.iterrows():
            t = format_tweet(row)
            question_text = clip_question_verbatim_screenshot(row['url'], './written_questions_text')
